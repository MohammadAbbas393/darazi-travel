...
export async function POST(req) {
  ...
  delete item.desc;
  const { data, error } = await supabase.from('destinations').insert(item).select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
...
export async function PUT(req) {
  ...
  delete item.desc;
  const { data, error } = await supabase.from('destinations').update(item).eq('id', body.id).select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
